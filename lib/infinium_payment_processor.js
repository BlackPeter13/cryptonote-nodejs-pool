...
function INFPay(transferCommands, callback) {
  ...
  async.waterfall([
      ...
      //cmd is used to store data in redis database. See -> ZRANGE Infinium:payments:addres 0 -1
        ...
        var worker_address=command.rpc.transaction.transfers[0].address
        var payment__id="";
        if(command.rpc.transaction.payment_id!=""){
          payment__id=config.poolServer.paymentId.addressSeparator+""+command.rpc.transaction.payment_id
        }
        command.redis.push(['hincrby', config.coin + ':workers:' + worker_address+payment__id, 'balance', -command.amount]);
        command.redis.push(['hincrby', config.coin + ':workers:' + worker_address+payment__id, 'paid', command.amount]);
  ...
  apiInterfaces.rpcWallet('create_transaction', command.rpc, function (error, result) {
      if (error || !result) {
          if(error.code && error.data){
            if(error.code==-302 && error.data.max_amount){
              // need to lower tx amount
              command.amount=error.data.max_amount
              command.rpc.transaction.transfers[0].amount=error.data.max_amount
              apiInterfaces.rpcWallet('create_transaction', command.rpc, function (error, result){
                if (error || !result) {
                ...
                log('info', logSystem, 'create_transaction ok after lowering tx amount, command.rpc: %j', [command.rpc]);
