...
if(config.InfiniumAlgo){
    if(config.InfiniumAlgo=="cn-zls") infinium_algo = 1;
    if(config.InfiniumAlgo=="cn-lite") infinium_algo = 2;
}
...
function getBlockTemplate(callback){
    apiInterfaces.rpcDaemon('getblocktemplate',
                            {reserve_size: MERGE_MINING_TAG_RESERVED_SIZE, wallet_address: config.poolServer.poolAddress, mining_algo: infinium_algo},
                            callback)
}
...
function jobRefresh(loop, callback){
    ...
    getBlockTemplate(function(error, result){
        ...
        if (!currentBlockTemplate || result.height != currentBlockTemplate.height) { //changed due to bug, when pool has higher height that blockchain
...
function processShare(miner, job, blockTemplate, childBlockTemplate, params){
    ...
    if (config.poolServer.mergedMining){
        if (hashDiff.ge(childBlockTemplate.difficulty)){
            ...
            var submit_cmd = 'submitblock', submit_body=[mergedBuffer.toString('hex')]
            if (config.is_child_amx_api){
                submit_cmd = 'submit_block'
                submit_body={blocktemplate_blob: mergedBuffer.toString('hex')}
            }
            apiInterfaces.rpcDaemon(submit_cmd, submit_body, function (error, result) {
                if (error){
