...
=========================
This repository contains the pool software and website for the old loudmining.com website.
...
#### Requirements
* This is for Ubuntu 20.04 you may need to adjust some things to your specific distro
* **NEVER RUN A POOL AS A ROOT USER**  **ALWAYS SETUP A REGULAR USER WITH SUDO PERMISSIONS**
  If your server is setup to login as root look into changing this and login with regular user.
  Here's how to add a user with sudo (admin) permissions:
```
# adduser username
```
  Fill in all the details if you wish, only need password... 
```
# usermod -aG sudo username_you_just_created
```
...
* libsodium -- it's just required... Install it!
* git is required to clone the repositories
```
$ sudo apt update && sudo apt upgrade -y
$ sudo apt install git libssl-dev libboost-all-dev libsodium-dev
```

* Coin daemon(s) (find the coin's repo and **build** latest version from **source**)

* Use NVM (Node Version Manager) to install NodeJS -- This is easy --

```
$ curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.3/install.sh | bash
$ nvm install <nodeversion>     -- replace <nodeversion> with the version that you want to run eg. nvm install 12.22.12 or nvm install lts/erbium
```

* [Redis](http://redis.io/) key-value store v2.6+
  Because this is the database that runs the entire pool, you want it to be as secure as possible. This means **building** from **source**.
  
  https://www.digitalocean.com/community/tutorials/how-to-install-redis-from-source-on-ubuntu-18-04  -- this still works on 20.04
  please heed this warning and don't use the distro release package.
  Dont forget to tune redis-server
  Add these lines to your /etc/rc.local and make it executable.
  But wait, you don't have a file called rc.local...  No worries we can easily make it.
  Just run:
```
sudo nano /etc/rc.local
```
  And paste all of this into it.
```
#!/bin/sh -e
#
# rc.local
...
```
  ...
  run this command to make it executable on boot:
```
...
```
$ sudo adduser poolusername --disabled-password --disabled-login
```
To login with this user : 
```
$ sudo su - poolusername
```

#### 1) Downloading & Installing
* This is to be done using the `pool user` that you created above.
  Switch to this user now
```
$ sudo su - poolusername
```
  Clone the repository and run `npm update` for all the dependencies to be installed:
```
$ git clone https://github.com/LOUD-Mining/cryptonote-nodejs-pool pool
$ cd pool
$ npm update
```
...
Copy the `/home/poolusername/pool/config_examples/COIN.json` file of your choice to
`/home/poolusername/pool/config.json` then overview each options and change any to match your preferred setup.
...
You could use this to start the pool, yes...
```
$ node init.js
```
But... There's a watchdog you can install to keep the pool operational even if it crashes.
Install it by running this from the pool/ directory:
```
$ npm install pm2 -g
```
Then to start up your pool you use this instead:
```
$ pm2 start init.js
```
Use this to look at the logs as they are coming in live...
```
$ pm2 logs
```
Read the instructions for PM2 it's very powerful.
